...
egrep "CPU|eth0" /proc/interrupts  2>&1 | tee -a cpu_intr_info.txt

cat /proc/net/softnet_stat  2>&1 | tee -a softIQR_info.txt

netstat -s  2>&1 | tee -a netstat_statistics.txt

netstat -i eno1 2>&1 | tee -a netstat_eno1_statistics.txt

ss  -npt 2>&1 | tee -a SR_queue.txt
