...
func main() {
	...
	for i := 0; i < *reqsNb; i++ {
		...
		if isRandomLeader { /*epaxos with random leader*/
			...
			if leader == -1 && leader2 == -1 {
				i--
				continue
			}
		}

		/* Send proposal to leader(s) */
		pilotErr = nil
		if leader >= 0 {
		...
		if *twoLeaders {
			for true {
				...
				if repliedCmdId != -1 && repliedCmdId < id {
					// update latency if this response actually arrived earlier
		...
		} else { // either multipaxos or epaxos
			for true {
				select {
