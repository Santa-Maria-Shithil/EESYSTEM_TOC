...
var maxRuntime *int = flag.Int("runtime", 1000, "Max duration to run experiment in second. If negative, stop after sending up to reqsNb requests")
...
var numKeys *uint64 = flag.Uint64("numKeys", 10000000, "Number of keys in simulated store.")
...
func main() {
	...
	getViewChan := make(chan int32, 1000)
	...
	if !*twoLeaders {
		...
		viewChangeChan = make(chan *View, 1000)
		for i := 0; i < N; i++ {
	...
	for !exptDone {
		...
		case pid := <-getViewChan:
			...
			writers[viewServer].WriteByte(genericsmrproto.GET_VIEW)
			getViewArgs.Marshal(writers[viewServer])
			if err := writers[viewServer].Flush(); err != nil {
				viewServer = rand.Intn(N)
			}
			if pid == int32(0) {
		...
		case newView := <-viewChangeChan:
			nv := len(viewChangeChan) + 1
			for i := 0; i < nv; i++ {
				if newView.PilotId < 2 && newView.ViewId >= views[newView.PilotId].ViewId {
