...
func NewReplica(id int, peerAddrList []string, thrifty bool, exec bool, dreply bool, beacon bool, durable bool) *Replica {
	...
	//register RPCs
	r.prepareRPC = r.RegisterRPC(new(weakepaxosproto.Prepare), r.prepareChan)
	r.prepareReplyRPC = r.RegisterRPC(new(weakepaxosproto.PrepareReply), r.prepareReplyChan)
	r.preAcceptRPC = r.RegisterRPC(new(weakepaxosproto.PreAccept), r.preAcceptChan)
	r.preAcceptReplyRPC = r.RegisterRPC(new(weakepaxosproto.PreAcceptReply), r.preAcceptReplyChan)
	r.preAcceptOKRPC = r.RegisterRPC(new(weakepaxosproto.PreAcceptOK), r.preAcceptOKChan)
	r.acceptRPC = r.RegisterRPC(new(weakepaxosproto.Accept), r.acceptChan)
	r.acceptReplyRPC = r.RegisterRPC(new(weakepaxosproto.AcceptReply), r.acceptReplyChan)
	r.commitRPC = r.RegisterRPC(new(weakepaxosproto.Commit), r.commitChan)
	r.commitShortRPC = r.RegisterRPC(new(weakepaxosproto.CommitShort), r.commitShortChan)
	r.tryPreAcceptRPC = r.RegisterRPC(new(weakepaxosproto.TryPreAccept), r.tryPreAcceptChan)
	r.tryPreAcceptReplyRPC = r.RegisterRPC(new(weakepaxosproto.TryPreAcceptReply), r.tryPreAcceptReplyChan)
...
func (r *Replica) fastClock() {
	for !r.Shutdown {
		time.Sleep(BATCH_INTERVAL) // 5 ms
		fastClockChan <- true
	}
}
...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case prepareS := <-r.prepareChan:
			prepare := prepareS.(*weakepaxosproto.Prepare)
		...
		case preAcceptS := <-r.preAcceptChan:
			preAccept := preAcceptS.(*weakepaxosproto.PreAccept)
		...
		case acceptS := <-r.acceptChan:
			accept := acceptS.(*weakepaxosproto.Accept)
		...
		case commitS := <-r.commitChan:
			commit := commitS.(*weakepaxosproto.Commit)
		...
		case commitS := <-r.commitShortChan:
			commit := commitS.(*weakepaxosproto.CommitShort)
			//got a Commit message
			dlog.Printf("Received Commit for instance %d.%d\n", commit.LeaderId, commit.Instance)
		...
		case prepareReplyS := <-r.prepareReplyChan:
			prepareReply := prepareReplyS.(*weakepaxosproto.PrepareReply)
			//got a Prepare reply
		...
		case preAcceptReplyS := <-r.preAcceptReplyChan:
			preAcceptReply := preAcceptReplyS.(*weakepaxosproto.PreAcceptReply)
		...
		case preAcceptOKS := <-r.preAcceptOKChan:
			preAcceptOK := preAcceptOKS.(*weakepaxosproto.PreAcceptOK)
			//got a PreAccept reply
		...
		case acceptReplyS := <-r.acceptReplyChan:
			acceptReply := acceptReplyS.(*weakepaxosproto.AcceptReply)
			//got an Accept reply
		...
		case tryPreAcceptS := <-r.tryPreAcceptChan:
			tryPreAccept := tryPreAcceptS.(*weakepaxosproto.TryPreAccept)
		...
		case tryPreAcceptReplyS := <-r.tryPreAcceptReplyChan:
			tryPreAcceptReply := tryPreAcceptReplyS.(*weakepaxosproto.TryPreAcceptReply)
	...
	for !r.Shutdown {
		...
		for q := 0; q < r.N; q++ {
			...
			for inst = r.ExecedUpTo[q] + 1; inst < r.crtInstance[q]; inst++ {
				if r.InstanceSpace[q][inst] != nil && r.InstanceSpace[q][inst].Status == weakepaxosproto.EXECUTED {
					if inst == r.ExecedUpTo[q]+1 {
						r.ExecedUpTo[q] = inst
					}
					continue
				}
				if r.InstanceSpace[q][inst] == nil || r.InstanceSpace[q][inst].Status != weakepaxosproto.COMMITTED {
...
func (r *Replica) replyPrepare(replicaId int32, reply *weakepaxosproto.PrepareReply) {
	r.SendMsg(replicaId, r.prepareReplyRPC, reply)
}

func (r *Replica) replyPreAccept(replicaId int32, reply *weakepaxosproto.PreAcceptReply) {
	r.SendMsg(replicaId, r.preAcceptReplyRPC, reply)
}

func (r *Replica) replyAccept(replicaId int32, reply *weakepaxosproto.AcceptReply) {
	r.SendMsg(replicaId, r.acceptReplyRPC, reply)
}

func (r *Replica) replyTryPreAccept(replicaId int32, reply *weakepaxosproto.TryPreAcceptReply) {
...
func (r *Replica) bcastPrepare(replica int32, instance int32, ballot int32) {
	...
	args := &weakepaxosproto.Prepare{r.Id, replica, instance, ballot}
...
var pa weakepaxosproto.PreAccept
...
var tpa weakepaxosproto.TryPreAccept
...
var ec weakepaxosproto.Commit
var ecs weakepaxosproto.CommitShort
...
func (r *Replica) updateCommitted(replica int32) {
	for r.InstanceSpace[replica][r.CommittedUpTo[replica]+1] != nil &&
		(r.InstanceSpace[replica][r.CommittedUpTo[replica]+1].Status == weakepaxosproto.COMMITTED ||
			r.InstanceSpace[replica][r.CommittedUpTo[replica]+1].Status == weakepaxosproto.EXECUTED) {
...
func (r *Replica) startPhase1(replica int32, instance int32, ballot int32, proposals []*genericsmr.Propose, cmds []state.Command, batchSize int) {
	...
	r.InstanceSpace[r.Id][instance] = &Instance{
		cmds,
		ballot,
		weakepaxosproto.COMMITTED, //@sshithil
		seq,
		deps,
	...
	if r.Id == 0 && DO_CHECKPOINTING && cpcounter >= CHECKPOINT_PERIOD {
		...
		r.InstanceSpace[r.Id][instance] = &Instance{
			cpMarker,
			0,
			weakepaxosproto.COMMITTED, //changed @sshithil
			r.maxSeq,
			deps,
...
func (r *Replica) handlePreAccept(preAccept *weakepaxosproto.PreAccept) {
	...
	if inst != nil && (inst.Status == weakepaxosproto.COMMITTED || inst.Status == weakepaxosproto.ACCEPTED) {
	...
	status := weakepaxosproto.PREACCEPTED_EQ
	if changed {
		status = weakepaxosproto.PREACCEPTED
	}

	if inst != nil {
		if preAccept.Ballot < inst.ballot {
			r.replyPreAccept(preAccept.LeaderId,
				&weakepaxosproto.PreAcceptReply{
					preAccept.Replica,
					preAccept.Instance,
					FALSE,
					inst.ballot,
	...
	if changed || uncommittedDeps || preAccept.Replica != preAccept.LeaderId || !isInitialBallot(preAccept.Ballot) {
		...
		pok := &weakepaxosproto.PreAcceptOK{preAccept.Instance}
...
func (r *Replica) handlePreAcceptReply(pareply *weakepaxosproto.PreAcceptReply) {
	dlog.Printf("Handling PreAccept reply\n")
	inst := r.InstanceSpace[pareply.Replica][pareply.Instance]

	if inst.Status != weakepaxosproto.PREACCEPTED {
	...
	if inst.lb.preAcceptOKs >= r.N/2+(r.N/2+1)/2-1 && inst.lb.allEqual && allCommitted && isInitialBallot(inst.ballot) {
		...
		r.InstanceSpace[pareply.Replica][pareply.Instance].Status = weakepaxosproto.COMMITTED
		...
		slow++
		inst.Status = weakepaxosproto.ACCEPTED
...
func (r *Replica) handlePreAcceptOK(pareply *weakepaxosproto.PreAcceptOK) {
	dlog.Printf("Handling PreAccept reply\n")
	...
	if inst.lb.preAcceptOKs >= r.N/2+(r.N/2+1)/2-1 && inst.lb.allEqual && allCommitted && isInitialBallot(inst.ballot) {
		happy++
		r.InstanceSpace[r.Id][pareply.Instance].Status = weakepaxosproto.COMMITTED
...
func (r *Replica) handleAccept(accept *weakepaxosproto.Accept) {
	...
	if inst != nil && (inst.Status == weakepaxosproto.COMMITTED || inst.Status == weakepaxosproto.EXECUTED) {
		return
	}
	...
	if inst != nil {
		if accept.Ballot < inst.ballot {
			r.replyAccept(accept.LeaderId, &weakepaxosproto.AcceptReply{accept.Replica, accept.Instance, FALSE, inst.ballot})
			return
		}
		inst.Status = weakepaxosproto.ACCEPTED
		inst.Seq = accept.Seq
...
func (r *Replica) handleAcceptReply(areply *weakepaxosproto.AcceptReply) {
	inst := r.InstanceSpace[areply.Replica][areply.Instance]

	if inst.Status != weakepaxosproto.ACCEPTED {
	...
	if inst.lb.acceptOKs+1 > r.N/2 {
		r.InstanceSpace[areply.Replica][areply.Instance].Status = weakepaxosproto.COMMITTED
...
func (r *Replica) handleCommit(commit *weakepaxosproto.Commit) {
	...
	if inst != nil {
		...
		inst.Status = weakepaxosproto.COMMITTED
	} else {
		r.InstanceSpace[commit.Replica][int(commit.Instance)] = &Instance{
			commit.Command,
			0,
			weakepaxosproto.COMMITTED,
			commit.Seq,
			commit.Deps,
			nil,
			0,
...
func (r *Replica) handleCommitShort(commit *weakepaxosproto.CommitShort) {
	inst := r.InstanceSpace[commit.Replica][commit.Instance]
...
func (r *Replica) startRecoveryForInstance(replica int32, instance int32) {
	...
	if r.InstanceSpace[replica][instance] == nil {
		r.InstanceSpace[replica][instance] = &Instance{nil, 0, weakepaxosproto.NONE, 0, nildeps, nil, 0, 0, nil}
	}
	...
	} else if inst.Status >= weakepaxosproto.PREACCEPTED {
...
func (r *Replica) handlePrepare(prepare *weakepaxosproto.Prepare) {
	inst := r.InstanceSpace[prepare.Replica][prepare.Instance]
	var preply *weakepaxosproto.PrepareReply
	var nildeps []int32
	...
	if inst == nil {
		r.InstanceSpace[prepare.Replica][prepare.Instance] = &Instance{
			nil,
			prepare.Ballot,
			weakepaxosproto.NONE,
			0,
			nildeps,
			nil, 0, 0, nil}
		preply = &weakepaxosproto.PrepareReply{
			r.Id,
			prepare.Replica,
			prepare.Instance,
			TRUE,
...
func (r *Replica) handlePrepareReply(preply *weakepaxosproto.PrepareReply) {
	...
	if preply.Status == weakepaxosproto.COMMITTED || preply.Status == weakepaxosproto.EXECUTED {
	...
	if preply.Status == weakepaxosproto.ACCEPTED {
	...
	if (preply.Status == weakepaxosproto.PREACCEPTED || preply.Status == weakepaxosproto.PREACCEPTED_EQ) &&
		(inst.lb.recoveryInst == nil || inst.lb.recoveryInst.status < weakepaxosproto.ACCEPTED) {
		...
		} else if preply.Status == weakepaxosproto.PREACCEPTED_EQ {
	...
	if ir != nil {
		...
		if ir.status == weakepaxosproto.ACCEPTED ||
			(!ir.leaderResponded && ir.preAcceptCount >= r.N/2 && (r.Thrifty || ir.status == weakepaxosproto.PREACCEPTED_EQ)) {
			...
			if conf, q, i := r.findPreAcceptConflicts(ir.cmds, preply.Replica, preply.Instance, ir.seq, ir.deps); conf {
				if r.InstanceSpace[q][i].Status >= weakepaxosproto.COMMITTED {
				...
				inst.Seq = ir.seq
				inst.Deps = ir.deps
				inst.Status = weakepaxosproto.PREACCEPTED
...
func (r *Replica) handleTryPreAccept(tpa *weakepaxosproto.TryPreAccept) {
	inst := r.InstanceSpace[tpa.Replica][tpa.Instance]
	if inst != nil && inst.ballot > tpa.Ballot {
		// ballot number too small
		r.replyTryPreAccept(tpa.LeaderId, &weakepaxosproto.TryPreAcceptReply{
			r.Id,
			tpa.Replica,
			tpa.Instance,
			FALSE,
	...
	if conflict, confRep, confInst := r.findPreAcceptConflicts(tpa.Command, tpa.Replica, tpa.Instance, tpa.Seq, tpa.Deps); conflict {
		...
		if inst != nil {
			...
			r.InstanceSpace[tpa.Replica][tpa.Instance] = &Instance{
				tpa.Command,
				tpa.Ballot,
				weakepaxosproto.PREACCEPTED,
				tpa.Seq,
				tpa.Deps,
				nil, 0, 0,
				nil}
		}
		r.replyTryPreAccept(tpa.LeaderId, &weakepaxosproto.TryPreAcceptReply{r.Id, tpa.Replica, tpa.Instance, TRUE, inst.ballot, 0, 0, 0})
	}
}
	...
	for q := int32(0); q < int32(r.N); q++ {
		for i := r.ExecedUpTo[q]; i < r.crtInstance[q]; i++ {
			...
			if state.ConflictBatch(inst.Cmds, cmds) {
				if i > deps[q] ||
					(i < deps[q] && inst.Seq >= seq && (q != replica || inst.Status > weakepaxosproto.PREACCEPTED_EQ)) {
					// this is a conflict
					return true, q, i
				}
			}
...
func (r *Replica) handleTryPreAcceptReply(tpar *weakepaxosproto.TryPreAcceptReply) {
	...
	if tpar.OK == TRUE {
		...
		if tpar.ConflictStatus >= weakepaxosproto.COMMITTED || notInQuorum > r.N/2 {
