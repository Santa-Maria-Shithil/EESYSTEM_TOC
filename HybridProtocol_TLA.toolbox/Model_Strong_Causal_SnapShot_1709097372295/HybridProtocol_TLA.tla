...
Message ==
  ...
  \cup  [type: {"accept"}, src: Replicas, dst: Replicas,
        inst: Instances, ballot: Nat \X Replicas,
        cmd: Commands \cup {[op |-> [key |-> "", type |-> ""]]}, deps: SUBSET Instances, seq: Nat, consistency: Consistency_level, ctxid:  Ctx_id \cup {0},  commit_order: Nat, clk: Nat]
  \cup  [type: {"commit"}, 
        inst: Instances, ballot: Nat \X Replicas,
        cmd: Commands \cup {[op |-> [key |-> "", type |-> ""]]}, deps: SUBSET Instances, seq: Nat, consistency: Consistency_level, ctxid:  Ctx_id \cup {0}, clk: Nat, commit_order: Nat]
  ...
  (* {[type |-> "commit", inst |-> <<1, 1>>, ballot |-> <<0, 1>>, cmd |-> [op |-> [key |-> "x", type |-> "r"]], 
   deps |-> {}, seq |-> 2, consistency |-> "causal", ctxid |-> 1, commit_order |-> 0, clk |-> 1], 
   [type |-> "commit", inst |-> <<1, 2>>, ballot |-> <<0, 1>>, cmd |-> [op |-> [key |-> "x", type |-> "w"]], 
   deps |-> {<<1, 1>>}, seq |-> 3, consistency |-> "causal", ctxid |-> 1, commit_order |-> 0, clk |-> 2], 
   [type |-> "commit", inst |-> <<1, 3>>, ballot |-> <<0, 1>>, cmd |-> [op |-> [key |-> "y", type |-> "r"]], 
   deps |-> {<<1, 1>>, <<1, 2>>}, seq |-> 4, consistency |-> "causal", ctxid |-> 1, commit_order |-> 0, clk |-> 3], 
   [type |-> "commit", inst |-> <<1, 4>>, ballot |-> <<0, 1>>, cmd |-> [op |-> [key |-> "y", type |-> "w"]], 
   deps |-> {<<1, 1>>, <<1, 2>>, <<1, 3>>}, seq |-> 6, consistency |-> "strong", ctxid |-> 1, commit_order |-> 2, 
   clk |-> 7], [type |-> "commit", inst |-> <<2, 1>>, ballot |-> <<0, 2>>, cmd |-> [op |-> [key |-> "w", type |-> "w"]], 
   deps |-> {}, seq |-> 2, consistency |-> "causal", ctxid |-> 1, commit_order |-> 0, clk |-> 1]}*)
           ...
           \/ /\ ~(\E rec \in cmdLog[replica] : rec.inst = msg.inst)
              /\ sentMsg' = (sentMsg \ {msg}) \cup
                            {[type  |-> "prepare-reply",
                              ...
                              commit_order |-> 0,
                              clk |-> clk[replica]]} (* commit_order 0 means not committed *)
...
PrepareFinalize(replica, i, Q) ==
    ...
    /\ \E rec \in cmdLog[replica] :
       ...
       /\ LET replies == {msg \in sentMsg : 
            ...
            maxClk == Max({msg.clk : msg \in replies})
            newClk == 1 + Max({clk[replica]} \cup {maxClk}) IN
            /\ clk' = [clk EXCEPT ![replica] = newClk]
...
ReplyTryPreaccept(replica) ==
    \E tpa \in sentMsg :
        ...
        /\ LET oldRec == {rec \in cmdLog[replica] : rec.inst = tpa.inst}
           newClk == 1 + Max({clk[replica]} \cup {tpa.clk}) IN
...
FinalizeTryPreAccept(cleader, i, Q) ==
    \E rec \in cmdLog[cleader]:
        ...
        /\ LET tprs == {msg \in sentMsg : msg.type = "try-pre-accept-reply" /\
                            ...
                            msg.ballot = rec.ballot} 
            maxClk == Max({msg.clk : msg \in tprs})
            newClk == 1 + Max({clk[cleader]} \cup {maxClk}) IN
            /\ clk'=newClk 
...
\* Last modified Wed Feb 28 00:11:33 EST 2024 by santamariashithil
