...
CONSTANTS Commands,  Replicas, MaxBallot, Consistency_level, Ctx_id, Keys
 ...
 SlowQuorums(r) == {{r} \cup s : s \in OneElementSubsetsR(r)} (*for 3 replicas use OneElementSubsetsR().
 for 5 replicas use TwoElementsSubsetsR().*)
 
 FastQuorums(r) == {{r} \cup s : s \in OneElementSubsetsR(r)}
...
Message ==
  ...
  \cup  [type: {"accept"}, inst: Instances,  ballot: Nat \X Replicas, cmd: Commands \cup {[op |-> [key |-> "", type |-> ""]]},
        deps: SUBSET Instances, seq: Nat, consistency: Consistency_level, ctxid:  Ctx_id \cup {0}, src: Replicas, dst: Replicas,
        commit_order: Nat, clk: Nat]
...
\* Last modified Fri Mar 01 16:51:09 EST 2024 by santamariashithil
